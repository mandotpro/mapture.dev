...
_Source of truth for all closed-value sets used across catalog schemas, comment tags, graph models, and validation. All enums are closed in v1 — adding a value requires updating both this file and the corresponding constant in `src/internal/graph/graph.go`._
